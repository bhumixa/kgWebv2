  ...
  implements ControlValueAccessor, OnInit, OnChanges
{
  ...
  emitValueChange(change: IonIntlTelInputModel | null) {
    if (!!change && Object.keys(change).length > 0) {
      let bool = false;
      try {
        bool = this.phoneUtil.isValidNumberForRegion(
        ...
        if (bool) {
          this.propagateChange({ ...change, bool });
        } else {
  ...
  ngOnInit() {
    ...
    if (this.defaultCountryiso) {
      this.country = this.getCountryByIsoCode(this.defaultCountryiso);
    } else {
      this.country = this.getCountryByIsoCode('us');
    }
  }
  ...
  fillValues(value: IonIntlTelInputModel) {
    ...
    //   bool = this.phoneUtil.isValidNumberForRegion(
    //     this.phoneUtil.parse(
    //       this.phoneNumber, this.country.isoCode.toUpperCase()
    //     ), this.country.isoCode.toUpperCase()
    //   );
    //   if (!bool) {
    // setTimeout(() => {
  ...
  onCodeChange(event: { component: NgSelectComponent; value: any }): void {
  ...
  public onCodeSearchCountries(event: {
    ...
    event.component.onCompositionStart();

    event.component.items = this.filterCountries(text);
    event.component.onCompositionEnd(text);
  }
  ...
  onNumberChange(): void {
    ...
    if (!this.phoneNumber || !isoCode) {
      ...
      const internationallNo = googleNumber
        ...
        : '';
      const nationalNo = googleNumber
        ? this.phoneUtil.format(googleNumber)
        : '';
  ...
  private getCountryByIsoCode(isoCode: string): CountryI {
    ...
    return null; // Added default return value when no country is found
  }
