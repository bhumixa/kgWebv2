...
@Component({
  selector: 'app-dia-comp-general-info',
  templateUrl: './dia-comp-general-info.component.html',
  styleUrls: ['./dia-comp-general-info.component.scss'],
})
export class DiaCompGeneralInfoComponent implements OnInit {
  public userType: any;
  public userData: any;
  public userId: any;
  ...
  public validations_formGeneralInfo: UntypedFormGroup;
  public countries = [
  ...
  async ngOnInit() {
    ...
    this.validations_formGeneralInfo = this.formBuilder.group({
    ...
    if (!!this.userData && !!this.userData.username) {
      this.validations_formGeneralInfo.controls['phone'].setValue(
    ...
    if (res.data.parameter) {
      ...
      if (para && para.general) this.profileData = para.general;
    }

    if (!!this.profileData) {
      // console.log(this.profileData)
      this.validations_formGeneralInfo.controls['name'].setValue(
        this.profileData.name
      );
      this.validations_formGeneralInfo.controls['lastname'].setValue(
        this.profileData.lastname
      );
      this.validations_formGeneralInfo.controls['email'].setValue(
        this.profileData.email
      );
      this.validations_formGeneralInfo.controls['rapnet'].setValue(
        this.profileData.rapnet
      );
      //this.validations_formGeneralInfo.controls.country.setValue(this.countries[1]);
      // let match = this.countries.filter(res => res.name == this.profileData.country.name);
      // this.validations_formGeneralInfo.controls.country.setValue(match[0]);
    }
  }
  ...
  validation_messages = {
    ...
    rapnet: [
      {
        type: 'minlength',
        message: 'Rapnet ID must be at least 4 characters long.',
      },
    ],
    phone: [
  ...
  async onSubmit(values) {
    ...
    let updateResponse = await this.databaseServiceService.updateUserProfile(
      updateObj
    );
