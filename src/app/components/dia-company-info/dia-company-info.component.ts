...
@Component({
  selector: 'app-dia-company-info',
  templateUrl: './dia-company-info.component.html',
  styleUrls: ['./dia-company-info.component.scss'],
})
export class DiaCompanyInfoComponent implements OnInit {
  ...
  constructor(
    ...
    this.validations_formAddAdress = this.formBuilder.group({
      ...
      country: new UntypedFormControl(this.countries[0], Validators.required),
    });
  }
  ...
  async fetchAddress(userId) {
    ...
    if (!!res.isSuccess) {
      this.addresses = res.data[0].listOfAddresses;
    }
  }
  ...
  async ngOnInit() {
    ...
    if (res.data.parameter) {
      ...
      if (para && para.company) {
        this.profileData = para.company;
      }

      if (para && para.userAccount && !this.profileData) {
        let distributioncenter =
          para.userAccount?.distributioncenter?.companyName;

        if (distributioncenter == 'Mumbai') {
          let match = this.countries.filter((res) => res.name == 'India');
          this.validations_formAddAdress.controls['country'].setValue(match[0]);
        }
      }
    }

    if (this.profileData) {
      this.validations_formAddAdress.controls['name'].setValue(
        this.profileData.name
      );
      this.validations_formAddAdress.controls['address'].setValue(
        this.profileData.address
      );
      this.validations_formAddAdress.controls['block'].setValue(
        this.profileData.block
      );
      this.validations_formAddAdress.controls['city'].setValue(
        this.profileData.city
      );
      this.validations_formAddAdress.controls['zip'].setValue(
        this.profileData.zip
      );
      this.validations_formAddAdress.controls['street'].setValue(
        this.profileData.street
      );
      this.validations_formAddAdress.controls['state'].setValue(
        this.profileData.state
      );
      if (!!this.profileData.country) {
        let match = this.countries.filter(
          (res) => res.name == this.profileData.country.name
        );
        this.validations_formAddAdress.controls['country'].setValue(match[0]);
      }

      if (this.profileData.businessType) {
        let businessType = this.profileData.businessType;
  ...
  validation_messages = {
    name: [{ type: 'required', message: 'Your company name is required.' }],
    address: [
      { type: 'required', message: 'Your company address is required.' },
    ],
    block: [{ type: 'required', message: 'Block no is required.' }],
    street: [{ type: 'required', message: 'Street address is required.' }],
    city: [
      { type: 'required', message: 'City is required.' },
      { type: 'pattern', message: 'City must contain only Alphabets.' },
    ],
    state: [
      { type: 'required', message: 'State is required.' },
      { type: 'pattern', message: 'State must contain only Alphabets.' },
    ],
    zip: [
  ...
  async onSubmit(values) {
    ...
    if (!this.businessType) {
      this._configService.presentToast('Please Select Terms', 'error');
      await this.databaseServiceService.hideSpinner();
      return;
    }
    values.businessType = this.businessType;
    let objToPost = {
      useid: this.userId,
      update: 'company',
      newValues: values,
    ...
    this._configService.profileUpdated('Profile Updated');
    if (this.addresses.length == 0) {
      let pin = this.validations_formAddAdress.get('zip').value;
      let country = this.validations_formAddAdress.get('country').value;
      let addressObj = {
        id: this.databaseServiceService.refCompanyId,
        fullName: this.validations_formAddAdress.get('name').value,
        phoneNo: this.userData.username.toString().replace('+', ''),
      ...
      let resAddress = await this.databaseServiceService.insertAddress(
        addressObj
      );
      this._configService.addressAdded('New Address');
    }

    await this.databaseServiceService.hideSpinner();

    this._configService.presentToast('Profile Saved', 'success');
  ...
  async getAreaByZipCode() {
    if (this.Country == 'India') {
      ...
      if (this.PinCode && this.PinCode != '') {
        ...
        if (res.isSuccess) {
          console.log(res);
          //this.validations_formAddAdress.get("country").setValue(res.data[0].state);
          if (
            res.data.length > 0 &&
            !!res.data[0].city &&
            !!res.data[0].state
            ...
            this.validations_formAddAdress
              .get('city')
              .setValue(res.data[0].city);
            this.validations_formAddAdress
              .get('state')
              .setValue(res.data[0].state);
            let match = this.countries.filter((res) => res.name == 'India');
            this.validations_formAddAdress.controls['country'].setValue(
              match[0]
            );
          }
        }
